      ...
      {sources && showSources && (
        <div className="mt-3 space-y-2">
          {sources.map((source, index) => (
            ...
            >
              <span className="flex-shrink-0 flex h-5 w-5 items-center justify-center text-[10px] font-semibold bg-primary/10 text-primary border border-primary/20 rounded mt-0.5">
                {index + 1}
              </span>
                ...
                {source.type === "JournalEntryRef" ? (
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">{source.entryDate}</span>
                    </div>
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">Week of {source.weekStartDate}</span>
                      <span className="text-xs text-muted-foreground">· {source.insightType}</span>
                    </div>
                    <p className="text-sm text-muted-foreground mt-1 line-clamp-2 leading-relaxed">
                      {source.summary}
                    </p>
              ...
              <ArrowRight className="h-4 w-4 text-muted-foreground/50 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 mt-0.5" />
            </div>
          ))}
        </div>
      )}
